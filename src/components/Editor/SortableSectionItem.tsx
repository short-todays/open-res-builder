...
export default function SortableSectionItem({ section, index, total }: SortableSectionItemProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`px-4 py-3 flex items-center gap-3 transition-all duration-200 ${isDragging ? 'bg-blue-50 shadow-md' : isOver ? 'bg-gray-50' : 'hover:bg-gray-50'
        }`}
    >
      {/* Drag Handle */}
      <div
      ...
      {isDragging && (
        <div className="flex-shrink-0 text-blue-500 text-xs font-medium px-2 py-1 bg-blue-100 rounded">
          Moving
        </div>
      )}
    </div>
